...
(function () {
  ...
  // Версию обёртки достаём из User-Agent. Формат дважды менялся: ранние
  // resize:none-сборки писали "MaccodNative-V166" (целое), а конфиг 1.96+
  // перешёл на "MaccodNative/1.97" (точечный). Парсим ОБА формата → целое NNN
  // (1.97→197, 1.66→166). Если не распарсить, bundleVer=0 и maccode-rn не
  // встаёт — а при resize:none это намертво оставляет композер под клавиатурой.
  var bundleVer = 0;
  var mInt = /MaccodNative-V(\d+)/i.exec(ua);
  if (mInt) {
    bundleVer = parseInt(mInt[1], 10) || 0;
  } else {
    var mDot = /MaccodNative\/(\d+)\.(\d+)/i.exec(ua);
    if (mDot) bundleVer = (parseInt(mDot[1], 10) || 0) * 100 + (parseInt(mDot[2], 10) || 0);
  }
